...
- [ ] The canvas is a whiteboard, not a sidebar. Why spatial persistence changes AI chat. | Source: notes/genui-platform-transition-plan.md | Pillar: ai-first-product | Status: outlined | Outline: notes/blog/outlines/should-ai-chat-have-a-whiteboard.md | Note: Picked because ai-first-product is still underrepresented in published posts and this raw idea had the clearest HN-friendly angle. The framing shifts from "add a canvas" to "chat is the wrong memory shape for decision work": a whiteboard is external cognition, not decoration.
...
- [ ] Your API docs are your new landing page: building for agent customers | Source: [[b2cc-agents-as-customers]], notes/API_notes/API-strategy.md | Pillar: api-architecture | Status: outlined | Outline: notes/blog/outlines/api-docs-new-landing-page.md
...
- [ ] The listing schema is the market: why a scraped catalog should become a publishing standard | Source: notes/big-ideas/2026-04-09-open-coffee-listing-standard.md | Pillar: api-architecture | Status: outlined | Outline: notes/blog/outlines/what-if-the-real-marketplace-is-the-listing-schema.md | Note: The bigger moat is not scraping more supplier sites. It is becoming the structured listing layer suppliers publish into and buyers or agents act through. Fresh evidence from PR #289 and #291: `/v1/catalog` is becoming the shared contract across public docs, API filters, and structured process metadata. Scraping becomes bootstrap + verification infrastructure instead of the whole product.
...
- [ ] Processing transparency is a schema problem, not a vocabulary problem | Source: notes/decisions/004-processing-transparency-schema-api.md; coffee-scraper PRs #208/#216; coffee-app PR #289 | Pillar: api-architecture | Status: raw | Note: The co-fermentation post exposed the problem, then the code turned it into a contract: base method, fermentation type, additives, duration, disclosure level, confidence, and evidence. Angle: markets cannot reward transparency until the API can represent it without pretending absence means none.
...
- [x] Co-fermentation in specialty coffee: fruit, hops, and experimental processing (Prime Green Coffee as case study) | Source: prime_green_coffee scraper data | Pillar: supply-chain | Status: published (PR #208, slug: co-fermentation-wrong-question) | Outline: notes/blog/outlines/published/co-fermentation-exposed-coffees-real-transparency-gap.md | Note: Published as "Is Co-Fermentation Cheating? Wrong Question." Post-publish drift flagged 2026-04-26: it still says the pipeline captures `processing` as one text field, but ADR-004 plus scraper PRs #208/#216 added structured processing transparency fields after the draft shipped. Update the post before heavily promoting it.
...
- [ ] Producer identity is market infrastructure, not listing copy | Source: coffee-scraper producer column and extractor work (PRs #209/#215) | Pillar: coffee-data-pipeline | Status: raw | Note: Supplier pages mix farms, producers, regions, marketing labels, and relationship prose. Extracting `producer` safely requires provenance and residue filters, not another regex. The broader lesson: comparison markets need entity identity before they can support trust, discovery, or relationship-level analytics.
...
- [ ] Wholesale as a data classification problem: price tiers, volume thresholds, and the retail/wholesale spectrum | Source: coffee-scraper PRs #47-#50, coffee-app PRs #56-#60, closed coffee-app PR #118 | Pillar: market-intelligence | Status: archived (closed PR #118; not current priority) | Outline: notes/blog/outlines/archive/why-same-coffee-costs-20-and-6.md | Draft snapshot: notes/blog/archive/drafts/why-same-coffee-costs-20-and-6.svx | Note: Reframed as "Why Does the Same Coffee Cost $20/lb and $6.58/lb?" The angle had promise, but the post still felt half-baked after red-team editing and does not reflect the current blog priority. Keep as source material for future pricing/quantity-tier work, not an active draft.
