...
value_quality  — lot's (score_value / cost_lb) is >= value_z_threshold standard
                 deviations above its origin+market baseline mean among scored,
                 stocked lots in the same market (retail and wholesale baselines
                 are computed separately, never pooled, because wholesale
                 minimum-tier pricing depresses cost_lb and would otherwise
                 inflate wholesale ratios against retail)
```

Defaults (configurable server-side, reported in responses): `drop_threshold_pct = 8`, `value_z_threshold = 1.5`, benchmark floor = segment must have ≥ 5 stocked lots from ≥ 3 suppliers or no segment-priced signal is emitted. `value_quality` also requires an origin+market scored-baseline floor evaluated per market: ≥ 5 scored stocked lots from ≥ 3 suppliers in that market, at least two distinct positive `score_value / cost_lb` ratios, and `origin_market_value_ratio_stddev > 0`; otherwise no `value_quality` row is emitted for that origin+market.
...
| -------------------------------------------------------------- | ----------------------------------------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| Anonymous / no credentials                                     | 401; `?summary=true` allowed (unfiltered counts only) | designated public summary only: no `origin`, no `process`, `market=retail` | 401 except the designated public slice: `dimension=process`, no `origin`, `market=retail`, `grain=month` |
| Viewer (session, no PPI)                                       | 403; `?summary=true` allowed (unfiltered counts only) | designated public summary only: no `origin`, no `process`, `market=retail` | 403 except the designated public slice: `dimension=process`, no `origin`, `market=retail`, `grain=month` |
| Parchment Intelligence session (`ppiAccess`)                   | full                                                  | full                                                                       | full                                                                                                     |
| API key: Green or Origin/Enterprise without `ppiAccess`        | 403 (`?summary=true` unfiltered allowed)              | designated public summary only: no `origin`, no `process`, `market=retail` | public slice only (`dimension=process`, no `origin`, `market=retail`, `grain=month`)                     |
| API key: Origin / Enterprise with `catalog:read` + `ppiAccess` | full                                                  | full                                                                       | full                                                                                                     |
| Admin                                                          | full                                                  | full                                                                       | full                                                                                                     |

Server-side enforcement per ADR-005/PADR-0013: API plan and API scopes do not grant Market Index/Parchment Intelligence by themselves. Premium signal, segment-stat, and metadata-index access requires `ppiAccess` (or admin), in addition to whatever API-key plan/scope is needed for the route. The word "market-wide" in this plan means **no `origin` filter**; it does not grant access to `market=all` or `market=wholesale`. Non-`ppiAccess` callers may only use the exact public slices above. Deny gated params at the boundary; 401 unauthenticated where auth is required, 403 for insufficient entitlement; no premium metadata in denied responses. Include tests proving direct calls cannot bypass gates, including an Origin/Enterprise API key with `catalog:read` but no `ppiAccess`.
...
CREATE TABLE market_signals (
  ...
  rank_score      numeric NOT NULL,        -- ordering key: signal strength x quality prior
...
1. **Signals pass** — for the latest `snapshot_date`: compute per-lot trailing medians (7d/30d) from `coffee_price_snapshots`; compute per-segment current-price distributions (p25/median) by joining stocked snapshot rows to `coffee_catalog` and grouping by non-null `country`/process bucket/market, where process bucket is `COALESCE(processing_base_method, 'undisclosed')`; compute origin+market-level scored-lot value-ratio baselines (`score_value / cost_lb` mean + stddev), grouping by `country`/market so retail and wholesale each get their own baseline, for `value_quality`, but only from positive-price scored stocked lots with non-null origin and only when that origin+market passes the §3.2 scored-baseline floor. Do not emit `below_market` or `value_quality` rows when `coffee_catalog.country IS NULL`, because those signals require a real origin/segment benchmark. Do not emit `value_quality` rows when the origin+market ratio count is thin, all ratios are identical, or `origin_market_value_ratio_stddev` is NULL/0. Emit `market_signals` rows per §3.2 rules with §3.3 evidence. Emit `price_drop` rows once per qualifying `signal_window` (`7d` and/or `30d` independently — a lot that qualifies in both produces two rows carrying that window's median/discount in `own_trailing_median` and `drop_vs_own_median_pct`); `below_market` and `value_quality` emit a single `signal_window='n/a'` row. Denormalize `origin` from `coffee_catalog.country` and `source` from `coffee_catalog.source` as-is (either may be NULL), and denormalize process as an explicit bucket (`Washed`, `Natural`, `undisclosed`, etc.); missing origin/source/process must not abort the daily pass, but missing origin suppresses segment-priced signals as above. Idempotent per (date): delete-and-rewrite the day's rows or upsert on the unique key (`snapshot_date, signal_type, signal_window, catalog_id, market`).
...
- **Auth/entitlement:** §3.5. For callers without `ppiAccess`, `?summary=true` bypasses the gate only for an unfiltered public teaser and returns `{ as_of, total, by_type: { price_drop: n, below_market: n, value_quality: n } }` only. If `summary=true` is combined with gated filters (`origin`, `process`, `market`, `type`, `window`, `min_discount_pct`, `min_score`) by a caller without `ppiAccess`, deny per the §3.5 matrix rather than returning filtered counts: 401 for anonymous/no-credential callers and 403 for authenticated sessions or API keys that lack entitlement. Callers with `ppiAccess` may request filtered summaries.
- **Params:** `type` (repeatable; default all three), `origin`, `process` (explicit process bucket, including `undisclosed`; omitted means all buckets), `market` (`retail|wholesale|all`, default `retail`; `all` means no market predicate over stored retail/wholesale signal rows), `min_discount_pct` (number), `min_score` (number), `window` (`7d|30d`, default `30d`; affects `price_drop` only), `limit` (default 20, max 100), `cursor`.
...
- **Tests:** entitlement matrix coverage incl. direct-URL bypass attempts and `catalog:read` API keys without `ppiAccess`; summary mode leaks no lot fields and rejects gated filters for callers without `ppiAccess`, returning 401 for anonymous callers and 403 for authenticated/API-key callers lacking entitlement; benchmark floor suppresses thin segments; missing-origin lots can emit `price_drop` when own-history rules pass but never emit `below_market` or `value_quality`; `market=all` returns the union of stored retail/wholesale rows with no synthesized all-market signal; scored-baseline floor suppresses `value_quality` rows when fewer than two distinct positive ratios exist or stddev is NULL/0; value_quality baselines are computed per origin+market so wholesale minimum-tier pricing never inflates ratios against retail; evidence shape is exactly §3.3 (nulls present), including 7d price-drop evidence and value-quality z-score inputs; each signal type has a non-null `rank_score_input`, `rank_signal_magnitude`, and `rank_score`, with `value_quality` sorted by `value_z_score` rather than segment discount.

### 4.5 Endpoint 2: `GET /v1/price-index/stats`

- **Auth/entitlement:** §3.5 — only the designated no-origin/no-process/`market=retail` public summary is public; segment queries and non-retail market summaries require `ppiAccess`.
...
- **Tests:** classification thresholds respected and returned from absolute-move percentile; default `baseline_weeks=12` produces classification with the launch history available from 2026-03-21; caller-requested baselines longer than available segment history return `available_baseline_weeks`, `classification: null`, null percentile/z-score fields, and `note: "insufficient_baseline_history"`; large positive and large negative moves with the same magnitude receive the same classification while preserving signed `latest_move_pct`; public summary computes from raw snapshots rather than averaging segment medians and is limited to the exact no-origin/no-process/`market=retail` slice; `process=undisclosed` is distinct from omitted process/all-process rollups; `insufficient_overlap` at `matched_lot_count < 8`; public callers can hit the designated summary but not segment queries or `market=wholesale|all`.
...
| `purvey market stats [--origin <o>] [--process <p>] [--market …] [--window 7d\|30d] [--baseline-weeks <n>] [--json]`                                                                      | `/v1/price-index/stats`     | Exact public summary (no origin/process, `market=retail`) works unauthenticated; rest requires entitlement                                         |
...
3. **P3 — Metadata index chapter** (needs endpoint 3 + backfill). New section under header "How is the market changing?": public proof chart = no-origin/`market=retail` process-mix trend (stacked area, `PROCESS_COLORS`); gated modules = origin-level disclosure trend, non-retail/all-market process trends, and score distribution band chart. Only after this ships: add "metadata index" positioning copy to `/analytics` + `/api` pages and BRAND.md naming if branded.
