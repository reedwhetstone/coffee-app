...
The documented procurement contract is caller-owned: `POST /v1/procurement/briefs` stores `user_id` from the authenticated caller, and the read/match routes reject a different owner. An operator token therefore cannot create a participant-owned brief by calling that endpoint. The pilot must use this private, operator-only setup path instead. The implementation must also ship the RLS hardening before the dashboard exposes PPI-owned cards: keep owner-scoped SELECT, but make authenticated INSERT/UPDATE/DELETE require a service-owned member/admin capability. Do not use a client-writable `user_roles` row as the authoritative source: revoke authenticated `user_roles` INSERT/UPDATE/DELETE, including the current owner-update policy, or use an equivalent non-client-writable entitlement source/security-definer capability function. The private control-plane/service-role seed operation remains the only writer for participant-owned rows. Add two-step negative coverage: a PPI-only session's attempted role promotion through Supabase REST is denied, and that same session still cannot create, mutate, or delete a brief through Supabase REST; member/admin writes and the private seed path retain their intended behavior.
...
- `supabase/migrations/<timestamp>_harden_sourcing_brief_writes.sql` plus focused Supabase REST/RLS negative coverage for the service-owned entitlement source, PPI-only, member/admin, and private seed callers
...
- A PPI-only authenticated session cannot promote itself through `user_roles` or insert, update, or delete `sourcing_briefs` through Supabase REST; the brief-write policy uses a service-owned/non-client-writable entitlement source, while member/admin write behavior remains supported and the private control-plane seed path can create the participant-owned row.
...
- RLS/API negative tests prove the two-step bypass is closed: a PPI-only session cannot promote itself through `user_roles` and then cannot create, mutate, or delete `sourcing_briefs` through Supabase REST. The tests also prove that member/admin writes and the private seed operation retain their intended behavior and that the brief-write policy reads only a service-owned/non-client-writable entitlement source.
