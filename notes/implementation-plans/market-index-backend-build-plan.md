...
2. **`compute_metadata_index(p_date date)`** — recompute the week+month periods containing `p_date` for dimensions `process|disclosure|purveyor_score|purveyor_score_confidence|purveyor_score_tier`; mean-of-daily-shares aggregation for categorical dimensions; percentile buckets for Purveyor Score and confidence; `undisclosed` bucket explicit; suppress origin-level rows `lot_count < 5`; market-wide rows always written; idempotent on `uq_metadata_index`. Supplier-stated `score_value` is subjective and must not feed the metadata index.
