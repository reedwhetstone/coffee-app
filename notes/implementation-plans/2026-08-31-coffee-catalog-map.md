...
- **MAP-POINT-SELECTION** | Every visible single-coffee point has a usable 44-pixel hit target and opens its entitled coffee directly from the feature identity, without depending on a second client-side lookup. | Canvas layer/dispatch regression tests for raw places and one-coffee semantic locations. | Proven on PR 5 current head | PR 5 interaction review.
- **MAP-DETAIL-CONTINUITY** | Selecting a coffee from a semantic location or visual cluster opens the canonical CoffeeCard without replacing the selected-area results with the global catalog; its mobile detail sheet stays below app chrome with a visible exit, and closing returns to the same area. | Selected-area detail/back-navigation and mobile layer/viewport regressions. | Proven on PR 5 current head | PR 5 interaction review and mobile preview feedback.
