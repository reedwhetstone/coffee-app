...
- **MAP-DETAIL-CONTINUITY** | Selecting a coffee from a semantic location or visual cluster opens the canonical CoffeeCard without replacing the selected-area results with the global catalog; closing the detail returns to the same area. | Selected-area detail/back-navigation component regression. | Proven on PR 5 current head | PR 5 interaction review.
