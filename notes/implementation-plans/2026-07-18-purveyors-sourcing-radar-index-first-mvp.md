...
- A PPI-only session cannot bypass concierge onboarding by inserting, updating, or deleting `sourcing_briefs` through Supabase REST; membership-aware RLS negative coverage proves the denial while preserving member/admin writes and the private participant seed path.
