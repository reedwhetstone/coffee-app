...
- New user-interaction analytics, event tokens, exposure/click tracking, watchlist attribution, product-event APIs, or event persistence.
...
The shared Supabase migration ledger is still a legacy coffee-app responsibility even though the canonical backend and authorization boundary have moved to the private Parchment workspace. Before PR 2 merges Parchment-owned schema changes, complete the separate Parchment plan `docs/plans/2026-07-18-database-migration-ownership-transfer.md` through its guarded canary.
...
PR 1 is read-only and may be developed in parallel after the publication gate. PR 2 may be developed against fixtures, but it cannot merge schema changes until the canonical Parchment migration path is operational.
...
- **Cross-surface effect:** The canonical read and owner-scoped intent contracts live in Parchment API and SDK. The dashboard and Parchment chat consume the same result without recomputing ranking or freshness.
...
- After PR 3: stop if the self-service product path cannot remain a thin client over the intent contract or becomes more complex than the sourcing need it captures.
- After PR 4 and the limited launch: refine or stop if customers report that Radar does not improve sourcing decisions, or if internal evidence audits show the signal is surfacing clearance rather than useful discovery.
...
The product provides value first. MVP confidence comes from product-output health, internal evidence audits, and customer conversations, not interaction tracking:
...
- **Evidence quality:** internal reconciliation confirms current price, availability, market scope, crop-age context, provenance, and source links against the accepted publication and supplier record.
- **Customer usefulness:** interviews ask whether Radar shortened discovery, improved a shortlist, or changed what the customer investigated.
- **Business:** customer willingness to keep paying and requests for recurring delivery are assessed through conversations rather than instrumentation or a checkout experiment.
...
- Customers report that the results do not change what they investigate or shorten their sourcing work.
...
The initial product direction is supported when multiple customers report that Radar repeatedly changes what they investigate, improves a shortlist, or saves meaningful sourcing time, while internal audits confirm the evidence is current and trustworthy. Exact launch thresholds should not pretend that five users produce statistical certainty.
...
- **The product collapses back into a research harness.** Add no tracking or validation controls whose primary purpose is learning rather than helping the customer source, investigate, track, or refine intent.
