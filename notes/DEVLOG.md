...
## Priority 0: Current Product Priorities (reconciled May 8, 2026)
...
- [ ] **Analytics Intelligence Product Reframe** - Reframe the authenticated product hierarchy around green coffee supply-chain intelligence first, with Mallard Studio as the personalization/workflow layer rather than the umbrella product. First wave is frontend-first: navigation taxonomy, Intelligence Home, analytics command-center layout, honest action CTAs, catalog-to-intelligence connective tissue, and Mallard Studio boundary cleanup. Plan: `notes/implementation-plans/2026-05-08-analytics-intelligence-reframe.md`.
- [ ] **Parchment Intelligence API + CLI Bridge** - With `GET /v1/price-index` shipped in PR #312 and the scraper-side price-index freshness guard shipped in coffee-scraper PR #251, finish the `purvey intelligence price-index` command/export and conversion docs, then verify the CLI and public docs point at the same entitlement-safe contract. Current plan: `notes/implementation-plans/2026-05-08-parchment-intelligence-cli-activation.md`; prior bridge plan: `notes/implementation-plans/2026-05-04-parchment-intelligence-cli-conversion-bridge.md`.
- [ ] **CLI API-Key Catalog Parity** - `purvey catalog search/get --include-proof` now uses the canonical `/v1/catalog?include=proof` API path with `PARCHMENT_API_KEY` / `PURVEYORS_API_KEY` support in purveyors-cli PR #108. Remaining parity work: move default read-only catalog commands and new intelligence commands onto canonical API contracts where semantics match, while keeping inventory, roast, sales, and tasting session-bound. Plan: `notes/implementation-plans/2026-04-29-cli-api-key-catalog-parity.md`.
...
- [ ] **Canonical Green Coffee Matching** - The beta `/v1/catalog/:id/similar` endpoint, member comparison UI, conservative threshold calibration, and bounded hard-gated `canonical-match-v1` response groups shipped in PRs #331, #332, #336, #347, and follow-up SQL fixes #348-#350. Next slices: `purvey catalog similar` canonical grouped output and coffee-app chat/agent tool alignment, then identity review foundation, scraper resolution pipeline, and canonical merged views. Programs: `notes/implementation-plans/2026-05-07-canonical-similarity-cli-agent-alignment.md`, `notes/implementation-plans/2026-05-06-canonical-match-disambiguation-and-performance.md`, and `notes/implementation-plans/2026-05-04-canonical-green-coffee-matching.md`.
- [ ] **Catalog Proof Coverage + Query Filters** - Turn shipped `proof-summary-v1` rows into a measurable intelligence surface with `GET /v1/catalog/proof-coverage` (coffee-app PR #335 is the in-flight endpoint), then add validated, capability-gated proof query filters for member sessions and paid API tiers. Plans: `notes/implementation-plans/2026-05-04-catalog-proof-coverage-query.md` and `notes/implementation-plans/2026-05-07-proof-query-filter-contract.md`.
- [ ] **Purveyor Score Rollout Discipline** - Stored Purveyor Score fields, trigger/backfill migration, card hierarchy, and methodology docs shipped in PRs #345 and #348-#349. Next product work: validate the migration/backfill path in production, keep score language pinned to metadata completeness rather than cup quality or supplier verification, and decide whether API/CLI score filters belong behind member/API capability gates after real catalog distribution is inspected.
- [ ] **Saved Sourcing Briefs + Procurement Recommendation Seed** - Build explicit buyer-authored sourcing criteria before automated recommendations: saved brief data model, criteria validator, manual match API, member workflow, CLI procurement commands, and shadow recommendation runs with no external notifications. Plan: `notes/implementation-plans/2026-05-05-saved-sourcing-briefs-procurement-seed.md`.
- [ ] **Open Coffee Listing Schema Validator** - Add the upstream compatibility contract for supplier/agent-authored listings before any direct-feed or supplier-account work: `GET /v1/listing-schema`, API-key-gated `POST /v1/listings/validate`, mapping hints, normalized preview, and docs that frame this as dry-run validation only. Plan: `notes/implementation-plans/2026-05-08-open-listing-schema-validator.md`.
- [ ] **Proof Passport Pilot Review** - coffee-scraper PR #246 shipped the private 100-row proof-passport pilot export, fixture tests, and local-only JSON/Markdown packet generation. Next product step: run and review pilot packets, decide which claim-family statuses can graduate into app/API/CLI proof-passport contracts, and keep public copy away from supplier-verification or regulatory claims until manual review proves the lane. Plan: `../coffee-scraper/notes/implementation-plans/2026-05-04-proof-passport-pilot-export.md`.
- [ ] **Certifications Taxonomy Schema** - Promote recurring Organic/Fair Trade/Rainforest/etc. evidence out of overloaded scraper text fields into a first-class `coffee_catalog.certifications` contract. Next scraper slice: deterministic high-confidence certification extraction for future scrape rows, production-column readiness guard, and write-path enablement before any historical backfill or public badge work. Scraper plans: `../coffee-scraper/notes/implementation-plans/2026-04-29-certifications-taxonomy-schema.md` and `../coffee-scraper/notes/implementation-plans/2026-05-07-certification-extraction-readiness.md`.

### Reconciled March and May P0 items

- **Catalog proof summaries:** deterministic proof summaries, evidence-safe `/v1/catalog?include=proof`, CoffeeCard proof badges, `purvey catalog search/get --include-proof`, and the private proof-passport pilot exporter shipped across coffee-app PR #327, purveyors-cli PR #108, and coffee-scraper PR #246. Remaining proof work is now coverage measurement, query filters, proof-passport pilot review, and copy discipline, captured above.
- **Purveyors Price Index / PPI:** the data foundation, analytics UI, entitlement fields, Stripe copy, `/v1/price-index` endpoint, and scraper freshness guard have shipped under Parchment Intelligence. The active gap is CLI access plus docs/conversion examples, captured above.
...
- [x] **API** - Add optional `include=proof` summaries to `GET /v1/catalog`, with explicit include validation and evidence-safe output for API, CoffeeCard, CLI, and agent surfaces. Shipped in coffee-app PR #327; CLI consumption shipped in purveyors-cli PR #108.
...
- [ ] **API** - Add `GET /v1/catalog/proof-coverage` and then proof query filters so process/provenance/freshness/pricing evidence quality becomes measurable and queryable before agents rely on it. Proof filter implementation should wait for PR #335 or intentionally branch from it, and must reject unsupported proof-looking params instead of silently no-oping. Plans: `notes/implementation-plans/2026-05-04-catalog-proof-coverage-query.md` and `notes/implementation-plans/2026-05-07-proof-query-filter-contract.md`.
- [ ] **API / Matching** - Extend the shipped beta `/v1/catalog/:id/similar` contract, member comparison UI, threshold calibration, and `canonical-match-v1` grouped output into CLI/agent workflows, identity review, scraper resolution, and canonical merged views. Keep legacy RPC compatibility debt explicit until callers migrate. Programs: `notes/implementation-plans/2026-05-06-canonical-match-disambiguation-and-performance.md` and `notes/implementation-plans/2026-05-04-canonical-green-coffee-matching.md`.
...
- [ ] **CLI Product Surface** - Keep coffee-app product docs aligned with purveyors-cli `origin/main` (`purvey` 0.17.0 source): command groups are auth, catalog, inventory, roast, sales, tasting, config, context, and manifest; exported subpaths include catalog, inventory, roast, sales, tasting, manifest, artisan, ai, and shared lib. Treat API-backed `--include-proof`, structured process filters, pasted auth callback fallback, shell-escaped roast file/directory paths, and raw Ctrl+C watch shutdown as shipped CLI behavior, not future gaps. The remaining similarity-specific CLI gap is canonical grouped `catalog similar` output, tracked in Priority 0.
...
- **40 live suppliers as of 2026-05-05** in coffee-scraper `origin/main` (`6320efa`). Sleepy Mango and Ally Open are intentionally inactive until public green-coffee inventory/JSON access returns.
...
- **Recent scraper moat work:** processing transparency persistence, producer column support, conditional field policies, proof-readiness audit scoring, AI-note denominator hardening, high-noise source policy tuning, and source-specific fixes for Coffee Shrub, Genuine Origin, Roastmasters, and processing/additive evidence extraction.
...
- `notes/implementation-plans/2026-05-04-parchment-intelligence-cli-conversion-bridge.md` - Current CLI price-index command and docs conversion bridge plan
- `notes/implementation-plans/2026-05-04-catalog-proof-coverage-query.md` - Proof coverage endpoint plan
- `notes/implementation-plans/2026-05-07-proof-query-filter-contract.md` - Capability-gated proof query filter contract plan
- `notes/implementation-plans/2026-05-04-canonical-green-coffee-matching.md` - Canonical green coffee matching program
- `notes/implementation-plans/2026-05-07-canonical-similarity-cli-agent-alignment.md` - Canonical similarity CLI/chat alignment program
