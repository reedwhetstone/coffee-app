...
> of 200 requests per UTC calendar month and up to 25 items per read collection
> response. Aggregates and complete atomic batch receipts retain their documented
> endpoint bounds.
...
- API usage communication must clearly distinguish the shared account allowance from per-key attribution, read collections from aggregates, and complete atomic batch receipts from ordinary read limits.
