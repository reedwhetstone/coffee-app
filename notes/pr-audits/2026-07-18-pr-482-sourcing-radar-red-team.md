...
> **Resolved 2026-07-18, same day:** a production sentinel query confirmed `scrape_runs` was absent while the later `20260714` migration was live — the manual process had skipped a file in the middle. The operator applied `20260713_market_publication_foundation.sql` to production and verified the sentinel objects. The finding's process half stands: the skip was invisible for five days, which converts the recovery program's guarded release workflow (its PR 2) from nicety to demonstrated need. The earliest-start note in the program plan was updated to reflect the deployed foundation; the builder, shadow evidence, and cutover remain unbuilt.
