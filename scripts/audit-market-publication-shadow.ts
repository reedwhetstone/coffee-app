...
async function loadReport(client: SupabaseClient, options: CliOptions): Promise<ShadowReport> {
	...
	const legacySegments = await fetchPaginated<LegacySegmentRow>(
		...
		(from, to) =>
			client
				...
				.select('snapshot_date,origin,process,grade,wholesale_only,supplier_count,price_median')
...
function humanSummary(report: ShadowReport): string {
	const lines = [
...
async function main(): Promise<void> {
	...
	if (missing.length > 0) {
		console.error(`VALIDATION_BLOCKED_ENV: ${COMMAND} — missing required ${missing.join(', ')}`);
		process.exitCode = 2;
		return;
	}
	...
	try {
		...
		process.exitCode = report.verdict.accepted ? 0 : 1;
	} catch (error) {
		console.error(classifyServiceError(error));
		process.exitCode = 2;
	}
}
