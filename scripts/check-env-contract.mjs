...
const requiredVars = mode === 'e2e' ? [...STATIC_VALIDATION_VARS, ...E2E_ONLY_VARS] : STATIC_VALIDATION_VARS;
const missingVars = requiredVars.filter((key) => {
  const value = combinedEnv[key];
  return value === undefined || value === '';
});

const satisfiedFromRepoFiles = requiredVars.filter((key) => key in repoEnv && !(key in process.env));
