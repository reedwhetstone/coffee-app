...
- **AI/ML**: OpenRouter (Qwen3) embeddings for semantic search/RAG; OpenRouter (`@preset/test-workhorse-agent`) for chat completions
...
Prereqs: Node `22.x` (see `package.json` engines) and PNPM. Setup: `corepack enable && corepack prepare pnpm@latest --activate`.
...
# or
pnpm test -- --testNamePattern "part of test name"
```
...
- Tests: mock external services (Supabase, Stripe) to keep tests deterministic and fast.
