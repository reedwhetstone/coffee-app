...
This repo contains the web app, the public catalog and analytics surfaces, the API dashboard, and the internal route layer that powers the first-party product. It also depends on `@purveyors/cli`, which is treated as a first-class interface to the same domain model.
...
- `/analytics` public market intelligence, with deeper views for authenticated users and `ppi-member` users
...
- `/api-dashboard` API keys, usage analytics, and docs handoff
...
Do not document the whole `/api/*` tree as a stable public contract. The public contract today is the catalog feed plus the dashboard and docs that support it.
...
- API dashboard docs gateway: `/api-dashboard/docs`
