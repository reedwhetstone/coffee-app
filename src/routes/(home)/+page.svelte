<script lang="ts">
	...
	// Redirect authenticated users to catalog
	if (session) {
		goto('/catalog');
	}
			...
			<div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
				...
				<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
					{#each data.data.slice(0, 6) as coffee}
						<div
							class="bg-background-primary-light ring-border-light rounded-lg p-4 opacity-90 shadow-sm ring-1"
						>
							<div class="flex items-start justify-between">
								<div class="flex-1">
									<h3 class="text-text-primary-light font-semibold">
										{coffee.name}
									</h3>
									...
									{#if coffee.ai_description}
										<p class="text-text-secondary-light mt-2 text-xs">{coffee.ai_description}</p>
									{/if}
								</div>
							...
							<div class="text-text-secondary-light mt-3 flex items-center justify-between text-xs">
								<span
