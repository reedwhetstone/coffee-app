<script lang="ts">
		...
		savedEventValueSeries.forEach((eventSeries, index) => {
			...
			const eventLine = line<{ time: number; value: number }>()
				...
				.defined((d) => {
					...
					if (!isValid) {
						console.warn('Invalid saved event value data point:', {
							d,
							adjustedTime,
							xPos,
							yPos,
			...
			try {
				...
				if (pathData && pathData !== 'M' && !pathData.includes('NaN')) {
					...
					console.warn(
						`Skipping invalid event line path for series ${eventSeries.event_string}:`,
						pathData
					);
				}
			} catch (error) {
				console.error(
					`Failed to generate event line path for series ${eventSeries.event_string}:`,
					error
				);
			}
		});
	...
	function renderProcessedDataControlEvents(
		...
		if (fanData.length > 0 && fanData.some((d) => d.value > 0)) {
			...
			console.log('No fan data to render:', {
				fanDataLength: fanData.length,
				hasPositiveValues: fanData.some((d) => d.value > 0)
			});
		}
		...
		if (heatData.length > 0 && heatData.some((d) => d.value > 0)) {
			...
			console.log('No heat data to render:', {
				heatDataLength: heatData.length,
				hasPositiveValues: heatData.some((d) => d.value > 0)
			});
		}
	}
	...
	function formatTime(timeMs: number, chargeTime: number): string {
		const relativeTime = (timeMs - chargeTime) / (1000 * 60); // minutes relative to charge
	...
	// Helper function to get milestone events at a specific time
	...
	onMount(() => {
		console.log('=== CHART ONMOUNT TRIGGERED ===');
		// Initial setup
		const containerWidth = chartContainer.clientWidth;
		...
		// Initial chart update handled by consolidated effect
...
{#if tooltipState.visible && tooltipState.data && !isDuringRoasting}
