		...
		{:else}
			<div class="space-y-4">
				{#each sortedBatchNames as batchName}
					<div
						class="rounded-lg border border-border-light bg-background-secondary-light shadow-md"
					>
						...
						>
							<h3 class="text-primary-light flex items-center gap-2 text-lg font-semibold">
								...
								<span class="break-words">{batchName}</span>
							</h3>
								...
								{#each sortedGroupedProfiles[batchName] as profile}
									...
									>
										<div class="flex flex-col gap-2 sm:flex-row sm:justify-between">
											<div>
												<h4 class="text-primary-light break-words text-base font-semibold">
													{profile.coffee_name}
												</h4>
											...
											{#if profile.notes}
												<div
													class="break-words text-left text-sm text-text-primary-light sm:text-right"
												>
													{profile.notes}
												</div>
											{/if}
