<script lang="ts">
	...
	function getRoastCompletionStatus(profile: any): { status: string; color: string; bgColor: string } {
	...
	function getBatchSummary(profiles: any[]) {
		...
		const avgYield = profiles.reduce((sum, p) => {
			if (p.oz_in && p.oz_out) {
...
<div class="w-full max-w-[100vw] overflow-x-hidden">
	{#if !sortedBatchNames || sortedBatchNames.length === 0}
		<div class="rounded-lg bg-background-secondary-light p-8 text-center ring-1 ring-border-light">
			...
			<p class="text-text-secondary-light">Start roasting to see your profile history and analytics here</p>
		</div>
	{:else}
		<div class="space-y-6">
			{#each sortedBatchNames as batchName}
				...
				<div class="rounded-lg bg-background-secondary-light ring-1 ring-border-light shadow-sm">
					...
					>
						<div class="flex items-center gap-3">
							<span class="text-background-tertiary-light transition-transform duration-200 {isBatchExpanded(batchName) ? 'rotate-90' : ''}">
								▶
							</span>
							<div class="text-left">
						...
						>
							<div class="grid gap-3 sm:grid-cols-2">
								{#each profiles as profile}
									...
									<div
										class="group relative rounded-lg bg-background-primary-light ring-1 ring-border-light transition-all hover:scale-[1.02] hover:ring-background-tertiary-light {currentRoastProfile?.roast_id === profile.roast_id
											? 'ring-2 ring-background-tertiary-light bg-background-tertiary-light/5'
											: ''}"
									>
										<!-- Main clickable area -->
										<button
											...
											aria-label="View roast profile for {profile.coffee_name} from {formatDateForDisplay(profile.roast_date)}"
											aria-pressed={currentRoastProfile?.roast_id === profile.roast_id}
										>
										<div class="flex items-start justify-between mb-3">
											<div class="flex-1">
												<h4 class="font-semibold text-text-primary-light group-hover:text-background-tertiary-light line-clamp-2">
													{profile.coffee_name}
												</h4>
												<div class="flex items-center gap-2 mt-1">
													...
													<span class="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium {completionStatus.bgColor} {completionStatus.color}">
										...
										<div class="grid grid-cols-2 gap-3 text-xs">
											...
											<div class="space-y-1">
												...
												<div class="font-medium {getRoastLevelColor(profile.end_temperature)}">
													{profile.end_temperature ? `${Math.round(profile.end_temperature)}°F` : 'N/A'}
												</div>
											</div>
										</div>
										...
										{#if profile.notes}
											<div class="mt-3 border-t border-border-light pt-3">
												<p class="text-xs text-text-secondary-light line-clamp-2">
													{profile.notes}
												</p>
											</div>
										{/if}
									...
									<div class="absolute top-2 right-2 flex items-center gap-1 rounded-md bg-background-primary-light/90 p-1 opacity-0 shadow-sm transition-opacity group-hover:opacity-100 backdrop-blur-sm">
										<button
											type="button"
