...
describe('/api/beans DELETE data boundary', () => {
	it('routes inventory create, update, and delete mutations only through Parchment', () => {
		...
		const mutationHandlers = routeSource.slice(
		...
		expect(mutationHandlers).toContain('createParchmentCatalogInventoryItem');
		expect(mutationHandlers).toContain('updateParchmentInventoryItem');
		expect(mutationHandlers).not.toContain('.from(');
		expect(mutationHandlers).not.toContain('addToInventory');
		expect(mutationHandlers).not.toContain('updateInventory');
		...
		expect(existsSync(resolve('src/lib/data/inventory.ts'))).toBe(false);
	});
