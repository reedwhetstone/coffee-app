...
export const load: PageServerLoad = async ({ url }) => {
	...
	const serviceData = {
		name: 'Purveyors API',
		description:
			'Normalized green coffee catalog access, market analytics, and a unified docs path for the Purveyors platform.',
		provider: 'Purveyors',
		...
		features: [
			...
			'Account-aware key and usage dashboard',
	...
	const pricingTiers = [
		{
			...
			features: ['200 requests per month', '25 rows per call', 'Dashboard access'],
			popular: false
		},
		{
			name: 'Roaster+',
	...
	const faqs = [
		...
		},
		{
			...
			answer:
				'Not yet. Analytics are a major product surface in the web app at /analytics, with deeper authenticated and ppi-member views, but they are not currently sold as a separate public API-key endpoint family.'
		},
		{
			question: 'Where do I generate keys and inspect usage?',
			answer:
				'Use /api-dashboard to create keys, review monthly usage, and confirm your current tier and limits.'
		},
		{
	...
	return {
		meta: buildPublicMeta({
			baseUrl,
			path: '/api',
			title: 'Purveyors API | Green Coffee Catalog and Market Intelligence',
			description:
				'Access the Purveyors catalog feed, market analytics, API dashboard, and unified API plus CLI docs.',
			keywords: [
				'green coffee API',
				'coffee catalog API',
			...
			ogTitle: 'Purveyors API',
			ogDescription:
				'Normalized green coffee catalog access, market analytics, and unified documentation for the Purveyors platform.',
			twitterTitle: 'Purveyors API',
			twitterDescription:
				'Catalog data, analytics, dashboard tooling, and CLI workflows for Purveyors.',
			type: 'product',
