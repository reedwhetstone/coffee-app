...
const LEGACY_CATALOG_API_CACHE_TTL = 60 * 60 * 1000;
...
function projectLegacyCatalogRow(row: Record<string, unknown>): Record<string, unknown> {
	...
	for (const column of CATALOG_API_COLUMNS.split(',').map((value) => value.trim())) {
...
function sortLegacyCatalogRows(rows: Record<string, unknown>[]): Record<string, unknown>[] {
...
export async function buildLegacyExternalCatalogResponse(
	...
	try {
		...
		if (
			legacyCatalogApiCache.data &&
			now - legacyCatalogApiCache.timestamp < LEGACY_CATALOG_API_CACHE_TTL
		) {
			let responseData = legacyCatalogApiCache.data;
			let isLimited = false;
			...
			const response = json(
				{
					...
					total_available: legacyCatalogApiCache.data.length,
					limited: isLimited,
					limit: rowLimit > 0 ? rowLimit : undefined,
					tier: principal.apiPlan,
					cached: true,
					cache_timestamp: new Date(legacyCatalogApiCache.timestamp).toISOString(),
					api_version: '1.0'
				},
		...
		const rows = sortLegacyCatalogRows(
			(await getPublicCatalog(createAdminClient(), CATALOG_API_COLUMNS)).map(
				projectLegacyCatalogRow
			)
		);
		legacyCatalogApiCache = {
			data: rows,
			timestamp: now
		};
