...
describe('applyBffCatalogCacheHeaders', () => {
	it('adds Vary: Cookie, Authorization for anonymous responses so shared caches key on credentials', () => {
		...
		expect(headers.get('Vary')).toContain('Authorization');
	});

	it('does not add Vary for authenticated (no-store) responses', () => {
		const headers = applyBffCatalogCacheHeaders(new Headers(), true);
		expect(headers.get('Cache-Control')).toBe('private, no-store');
		expect(headers.get('Vary')).toBeNull();
	});

	it('merges credential tokens into an existing Vary header without duplicating', () => {
