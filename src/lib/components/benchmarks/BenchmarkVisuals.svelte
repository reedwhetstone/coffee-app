		...
		>
			<figcaption>
				...
				<p class="mt-1 text-xs leading-5 text-muted">
					The horizontal scale is the precomputed Bradley–Terry score from pairwise jury
					preferences, a relative measure rather than percent correct. The dot is the score; the
					line is its source-family clustered 95% interval.
				</p>
			...
			{#each tradeoffs as tradeoff (tradeoff.id)}
				...
				>
					<figcaption>
						<div class="flex flex-wrap items-center justify-between gap-2">
							<h3
								id={`tradeoff-${trackResult.track}-${tradeoff.id}`}
							...
							{#each tradeoff.points as point, index (point.subjectId)}
								...
								>
									...
									<span
										...
										data-vertical={tooltipVertical(
											pointPosition(point.value, tradeoff.valueBounds)
										)}
										role="tooltip"
									>
