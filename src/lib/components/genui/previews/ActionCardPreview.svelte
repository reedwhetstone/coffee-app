...
<button
	onclick={handleClick}
	class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-medium ring-1 ring-inset transition-all {statusColors[block.data.status] || statusColors.proposed}"
>
	<svg class="h-3.5 w-3.5 {block.data.status === 'executing' ? 'animate-spin' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
		<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d={statusIcons[block.data.status] || statusIcons.proposed} />
	</svg>
