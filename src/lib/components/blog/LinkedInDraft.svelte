...
<div class="rounded-lg border border-line bg-surface-canvas p-6 shadow-sm">
	<div class="mb-4 flex items-center justify-between">
		<div class="flex items-center gap-2">
			...
			<h3 class="text-sm font-semibold text-ink">LinkedIn Draft</h3>
		</div>
		{#if onclose}
			<button
				onclick={onclose}
	...
	<div class="mb-4 rounded border border-line bg-surface-panel p-4">
		<p class="whitespace-pre-line text-sm text-ink">{linkedInText}</p>
	</div>

	<div class="flex items-center gap-3">
		<button
			onclick={copyToClipboard}
			class="inline-flex items-center gap-1.5 rounded-md border border-line px-3 py-1.5 text-sm font-medium text-ink transition-all hover:border-accent hover:text-accent"
		>
			{#if copied}
				<svg
