<script lang="ts">
	...
	let signalTotal = $derived(Math.max(snapshot.totalSignals, 1));
...
<div class="space-y-8">
	...
	{#if marketRead}
		...
		>
			<div class="grid lg:grid-cols-[minmax(0,1.25fr)_minmax(17rem,0.75fr)]">
				...
				<article class="overflow-hidden rounded-xl border border-line bg-surface-panel shadow-sm">
					...
					>
						<div class="p-5 sm:p-6">
							...
							<p class="mt-1 text-sm text-muted">{coffee.supplier}</p>
