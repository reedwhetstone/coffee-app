	...
	},
	{
		...
		sections: [
			...
			},
			{
				...
				bullets: [
					'POST /api/beans sends catalog-backed inventory through the Parchment inventory contract and manual multi-row purchases through one atomic Parchment batch. The retired scalar manual payload is rejected. Batch creation and GET /api/beans?manualBatchId=<uuid> both require a Parchment Intelligence or Mallard Studio session; after an uncertain response, that GET reconciles the durable batch result.',
					'POST and PUT /api/beans adapt the web form into the owner-scoped Parchment inventory contract. PUT accepts an If-Match precondition for conflict-safe updates.',
