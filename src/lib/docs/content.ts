		...
		sections: [
			{
				...
				body: [
					'create-checkout-session accepts purchaseKey or purchaseKeys, rejects unknown or non-self-serve entries, and returns 409 when the request mixes same-family plans or conflicts with an existing active subscription. When Checkout admissions are enabled, the server also requires requestId as a UUID. Generate it once for a purchase selection and preserve that same value across ambiguous retries; changing the purchase selection requires a new requestId. Managed success responses echo requestId at the top level. Ambiguous 503 responses echo it as error.requestId so the caller can retry safely. Terminal errors may omit it and require a new requestId.',
